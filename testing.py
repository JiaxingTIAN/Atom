...
name = sys.stdin.readline()
print("Hello %s my favorite number is %.2f" % (name, 5.20))

testFile = open("test.txt", "wb")
testFile.write(bytes("This is a text file.\n", "UTF-8"))
testFile.close()

readfile = open("test.txt", "r+")   # Read and write file
...
print(function_sum(1, 4))


# Construct a class extend from object -- new style
class Animal(object):
    __name = None   # __ indicate private member

    # Constructor
    #functions in an class have to include self as a parameter
    def __init__(self, animal_name):
        self.__name = animal_name

    def set_name(self, ani_name):    # Refer to itself like this
    ...
    def tostring(self):
        print("{} is the animal name".format(self.__name))
...
class Dog(Animal):
    ...
    def __init__(self, owner, animal_name):
        self.__owner = owner
        super(Dog, self).__init__(animal_name)

    # Overload function, how_many is not really needed
    def sound(self, how_many=None):
...
del ages['Sue'] #delete the entry
print ages
ages.clear()    #clear all entry
del ages        #delete the dictionary